...
def generate_qr_codes_page(pages_data: Dict[str, Dict[str, str]]) -> None:
    ...
    with open(qr_page_path, 'w', encoding='utf-8') as f:
        ...
        f.write("Print these QR codes and attach them to physical equipment or locations.\n\n")
        f.write("!!! tip\n")
        f.write("    Download the QR codes from the `qr-codes/` directory in the repository.\n\n")
        ...
        for directory, items in sorted(grouped.items()):
            ...
            for md_path, data in items:
                ...
                f.write(f"![QR Code for {data['title']}](../{data['qr_path']}){{ width=\"300\" }}\n\n")
